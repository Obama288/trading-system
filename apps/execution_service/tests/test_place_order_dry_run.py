...
def test_place_order_dry_run_blocked_by_kill_switch():
...
def test_place_order_dry_run_duplicate_request():
    store = InMemoryExecutionStore()
    ks = StubKillSwitchClient(trading_enabled=True)

    first = place_order_dry_run_use_case(
        candidate_id="cand_001",
        execution_candidate=make_candidate(),
        execution_idempotency_key="idem_001",
        correlation_id="corr_001",
    ...
    second = place_order_dry_run_use_case(
        candidate_id="cand_001",
        execution_candidate=make_candidate(),
        execution_idempotency_key="idem_001",
...
def test_place_order_dry_run_accepted():
    store = InMemoryExecutionStore()
    ks = StubKillSwitchClient(trading_enabled=True)
...
def test_place_order_dry_run_rejects_empty_idempotency_key():
    store = InMemoryExecutionStore()
    ks = StubKillSwitchClient(trading_enabled=True)

    with pytest.raises(ValueError, match="execution_idempotency_key is required"):
        place_order_dry_run_use_case(
            candidate_id="cand_001",
            execution_candidate=make_candidate(),
