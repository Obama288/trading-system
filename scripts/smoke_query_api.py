...
async def run_query_api_smoke(
    *,
    settings: Any | None = None,
    client_factory: Callable[[Any], Any] | None = None,
) -> tuple[int, dict[str, Any]]:
    """Run only the B2c.1b query-api preflight and return sanitized output."""
    ...
    try:
        ...
        api_key_info = await client.get_query_api_info()
        elapsed_ms = int((time.perf_counter() - started) * 1000)
        output = {
            "endpoint": ENDPOINT_NAME,
            "status": "success",
            "exchange": api_key_info.exchange,
            "read_only": api_key_info.read_only,
            "permissions_safe": api_key_info.permissions_safe,
            "key_active": api_key_info.key_active,
            "deadline_days_present": api_key_info.deadline_days_present,
            "expired_at_present": api_key_info.expired_at_present,
            "elapsed_ms": elapsed_ms,
        }
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the Stage 53-B2c.1b Bybit query-api read-only preflight.",
    )
    parser.add_argument(
        "--pretty",
        action="store_true",
...
async def _async_main(argv: list[str] | None = None) -> int:
    ...
    if not args.allow_real_smoke:
        ...
        exit_code, output = await run_query_api_smoke()
