...
async def run_wallet_balance_smoke(
    *,
    settings: Any | None = None,
    client_factory: Callable[[Any], Any] | None = None,
) -> tuple[int, dict[str, Any]]:
    """Run only the B2c wallet_balance smoke and return sanitized output.

    This harness is real-capable, but B2c tests must mock the client/HTTP layer.
    It must not call server_time, open_positions, order_status, or write/live
    methods.
    """
    ...
    try:
        ...
        output.update(
            {
                "exchange": wallet_balance.exchange,
                "account_type": wallet_balance.account_type,
                "coins_count": len(wallet_balance.coins),
            }
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the Stage 53-B2c Bybit wallet_balance read-only smoke.",
    )
    parser.add_argument(
        "--pretty",
        action="store_true",
...
async def _async_main(argv: list[str] | None = None) -> int:
    ...
    if not args.allow_real_smoke:
        ...
        exit_code, output = await run_wallet_balance_smoke()
